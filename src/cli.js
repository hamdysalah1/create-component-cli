...
async function writeComponent(options) {
  ...
  if (options.styledComponent === "yes") {
    ...
    fs.writeFileSync(
      `${process.cwd()}/components/${compName}/styles/${compName}Style.js`,
      stylesByStyledComponent()
    );
  }
  ...
  if (options.type === "Component Object Model") {
    ...
    fs.writeFileSync(
      ...
      comComponent(compName, options.propsType, options.styledComponent)
    );
  }
}
